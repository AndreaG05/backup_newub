...
;;; Commentary:
;; Framework Emacs Configuration
;; Author: Leonardo Tamiano
;; Using package.el instead of straight.el
...
(use-package org
  ...
  (setq org-hidden-keywords '(title subtitle author))
...
;;     (vterm-send-string "clear && printf '\e[3J'")
;;     (vterm-send-return)))
...
useful for example when watching YouTube videos."
  ...
  (let ((url (elfeed-entry-link entry)))
    (when url
      ...
      ;; Remember to save position on quit.
...
(custom-set-variables
 ...
 '(package-selected-packages
   '(beacon better-shell corfu denote doom-modeline ef-themes elfeed emms
            eterm-256color ewal-spacemacs-themes expand-region
            ivy-posframe ivy-prescient magit multi-vterm ox-hugo
            popwin projectile spacemacs-theme standard-themes
            yasnippet)))
(custom-set-faces
